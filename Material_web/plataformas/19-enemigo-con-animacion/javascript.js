class Escena extends Phaser.Scene {
    ...
    create() {
        ...
        const hormigaFromTiled = this.findObjectsByType('hormigaEnemy', map);
        const enemies = this.physics.add.group({
            classType: 'hormiga',
            runChildUpdate: true
        });

        for (let i = 0; i < hormigaFromTiled.length; i++) {
            const malo = new HormigaEnemy(hormigaFromTiled[i].x, hormigaFromTiled[i].y, this);
            enemies.add(malo);
        }
