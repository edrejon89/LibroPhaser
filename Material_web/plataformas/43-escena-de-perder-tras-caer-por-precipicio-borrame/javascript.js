class Escena extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.spritesheet('player', '../img/transparente.png', {
            frameWidth: 180,
            frameHeight: 180
        });
    ...
    create() {
        ...
        this.player = new Player(this, playersFromTiled[0].x, 0);
        ...
        this.physics.add.overlap(this.player, avispas, this.player.checkEnemy, null, this.player);
        ...
        for (let i = 0; i < 32; i++) {
            for (let j = 0; j < 32; j++) {
                const c1 = this.add.zone(i * 32, j * 32, 32, 32)
                this.add.graphics().lineStyle(2, 0xff0000).strokeRectShape(c1);
            }
        }
...
class Player extends Phaser.Physics.Arcade.Sprite {
    ...
    caminarALaIzquierda() {
        if (!this.estaAturdido) {
            ...
            this.flipX = true;
            if (this.enElSuelo) this.play('player_caminar', true);
    ...
    playerGolpeadoPorAvispa() {
        ...
        this.scene.time.addEvent({
            delay: 1000,
            callback: this.terminoElAturdimiento,
            callbackScope: this
        });
    }
